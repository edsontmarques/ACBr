...
type
  ...
  published
    ...
    property CacheDiasValidade: Integer read fCacheDiasValidade
      write fCacheDiasValidade default 0;  // 0-n�o expira
...
begin
  ...
  try
    ...
    if (SL.Count <> 7) then
      raise EACBrIBGEException.CreateFmt( ACBrStr('Linha de UF inv�lida: %s'), [AValue] );
...
begin
  ...
  try
    ...
    if (SL.Count <> 6) then
      raise EACBrIBGEException.CreateFmt( ACBrStr('Linha de Cidade inv�lida: %s'), [AValue] );
    ...
    try
      ...
      begin
        ...
        begin
          ...
          iCidade := Find(CodMun * 10, False);   // Municipo vem sem o d�gito verificador
          if (iCidade >= 0) then
          begin
...
begin
  ...
  fCacheLido := False;  // For�a recarga do Cache, no formato correto
end;
...
begin
  ...
  if ACodMun = 0 then
     raise EACBrIBGEException.Create( ACBrStr('C�digo do Munic�pio deve ser informado') );
...
begin
  ...
  if (Trim(ACidade) = '') then
    raise EACBrIBGEException.Create( ACBrStr('Nome do Munic�pio deve ser informado') );

  if (Trim(AUF) <> '') then
  begin
    CodUF := UFToCodUF(Trim(AUF));
    if (CodUF < 0) then
      raise EACBrIBGEException.CreateFmt( ACBrStr('UF %s n�o encontrada'), [AUF] );
...
begin
  ...
  if not Carregar then                                    // J� carregou todas ?
    Exit;
...
begin
  ...
  if (iUF < 0) then
    raise EACBrIBGEException.CreateFmt( ACBrStr('C�digo da UF: %d inv�lido'), [ACodUF] );

  oUF := fListaUFs[iUF];
  if oUF.CidadesCarregadas then   // J� Carregou essa UF ?
    Exit;
...
begin
  ...
  if CodUF > 0 then
    ...
    raise EACBrIBGEException.CreateFmt( ACBrStr('UF: %s inv�lida'), [AUF] );
end;
...
begin
  ...
  if (AFile = '') then
    raise EACBrIBGEException.Create( ACBrStr('Nome do Arquivo n�o informado') );

  if (not Overwrite) and FileExists(AFile) then
    raise EACBrIBGEException.CreateFmt( ACBrStr('Arquivo %s j� existente'), [AFile] );
...
begin
  ...
  if (fListaUFs.Count < 1) then
    raise EACBrIBGEException.Create( ACBrStr('Tabelas de UF n�o est� na mem�ria') );

  if (fListaCidades.Count < 1) then
    raise EACBrIBGEException.Create( ACBrStr('Tabelas de Cidades n�o est� na mem�ria') );
...
begin
  ...
  if (fListaUFs.Count >= CIBGE_UF_COUNT) then  // J� fez a carga ?
    Exit;
